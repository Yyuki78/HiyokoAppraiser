...
public class TutrialHiyokoMove : MonoBehaviour
{
    private enum State
    {
        Normal,
    ...
    [SerializeField] bool EnableMove = false;
    private int clickNum=0;//始祖がクリックされた回数
    ...
    void Start()
    {
        ...
        //this.gameObject.transform.localScale = new Vector3(1, 1, 1f);
    }
    ...
    // 始祖がクリックされたときに呼び出されるメソッド
    public void OnPointClickSiso()
    {
        if (state == State.Finish || state == State.Die) return;
        if (clickNum > 50) return;
        clickNum++;
        print($"オブジェクト {name} がクリックされたよ！");
        state = State.Pick;
